...
in
{
  ...
  config = let
    forEachEnabledNmh = fn: flatten (mapAttrsToList (lib.optional cfg.nativeMessagingHosts.${k} (fn k v)) fn nmhOptions);
  in mkIf cfg.enable {
    ...
    environment.systemPackages = [
      (cfg.package.override (old: {
        extraPrefs = old.extraPrefs // cfg.autoConfig;
        ...
        in oldCfg // nixpkgsConfig // optionConfig;
      }))
    ];
